      ...
      <div className="grid w-full grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_220px] lg:gap-16">
        <section className="flex min-w-0 flex-col gap-6 lg:flex-row lg:items-center lg:gap-7">
          <div className="flex min-w-0 flex-1 flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <nav
              aria-label="Právní odkazy"
              className="flex flex-wrap items-center gap-x-7 gap-y-3 lg:justify-end"
            >
              {legalLinks.map((link) => (
                <Link
                  key={link.label}
                  to={link.href}
                  className="[font-family:'Inter',Helvetica] text-base font-normal leading-[31.5px] text-[#f2f4f7] underline decoration-from-font underline-offset-2 transition-opacity hover:opacity-80 sm:text-[18px] lg:text-[21px] whitespace-nowrap"
                >
                  {link.label}
                </Link>
              ))}
